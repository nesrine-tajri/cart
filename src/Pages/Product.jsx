...
function Product({ order, handleAddToCard,handleRemoveFromCard,def }) {
  

const sum = order.reduce((acc,x)=>acc+x.total,0)
...
const imageType = getImageType()
  return (
    <div className="  flex p-0 pt-14 justify-center sm:flex-col  bg-gray-100">
      <div className="flex flex-col space-x-20  justify-center gap-8 items-center p-0 m-0 text-left w-full ">
        <h1 className="font-bold text-left text-3xl">Desserts</h1>

        <div className="grid lg:grid-cols-3 gap-3 md:grid-cols-2 sm:grid-cols-1  ">
          {data.map((item, i) => (
            <Item key={i} {...item} handleRemoveFromCard={handleRemoveFromCard} handleAddToCard={handleAddToCard}  order={order}/>

          ))}
        </div>
      </div>
      <div className=" mr-20 w-2/6" >
      <h1 className="text-orange-700 font-bold text-xl my-6">Your Cart ({order.reduce((acc, item) => acc + item.quantity, 0)})</h1>
       {order.length > 0 ? (
        <div className="bg-white p-1 rounded-md shadow-sm">
          {order.filter(item => item.quantity > 0).map((item, i) => (
            <div key={i} className="bg-grey-50">
              <h2>{item.name}</h2>
              <div className="flex justify-between">
                <div className="flex gap-5">
                <p className="text-orange-700 font-semibold">{item.quantity}x</p>
                <p className="text-gray-500">@{item.price}  <span className="text-gray-700 ml-4">${item.total}</span></p>
                </div>
                <div className="rounded-3xl border-solid text-center items-center cursor-pointer border-gray-300 border-2 p-1 shadow-sm bg-orange-50 m-1" onClick={() => def(item.name)}>
          ...
          <h1 className="font-bold text-lg">${sum}</h1>
        </div>
        <div className="text-center items-center">
          <div className="flex  justify-center bg-orange-50 mx-6 rounded-xl my-3 shadow-sm items-center">
          <svg xmlns="http://www.w3.org/2000/svg" width="21" height="20" fill="none" viewBox="0 0 21 20"><path fill="#1EA575" d="M8 18.75H6.125V17.5H8V9.729L5.803 8.41l.644-1.072 2.196 1.318a1.256 1.256 0 0 1 .607 1.072V17.5A1.25 1.25 0 0 1 8 18.75Z"/><path fill="#1EA575" d="M14.25 18.75h-1.875a1.25 1.25 0 0 1-1.25-1.25v-6.875h3.75a2.498 2.498 0 0 0 2.488-2.747 2.594 2.594 0 0 0-2.622-2.253h-.99l-.11-.487C13.283 3.56 11.769 2.5 9.875 2.5a3.762 3.762 0 0 0-3.4 2.179l-.194.417-.54-.072A1.876 1.876 0 0 0 5.5 5a2.5 2.5 0 1 0 0 5v1.25a3.75 3.75 0 0 1 0-7.5h.05a5.019 5.019 0 0 1 4.325-2.5c2.3 0 4.182 1.236 4.845 3.125h.02a3.852 3.852 0 0 1 3.868 3.384 3.75 3.75 0 0 1-3.733 4.116h-2.5V17.5h1.875v1.25Z"/></svg>
            <p className="text-center items-center">this is a <span>carbon-neutral</span> delivery</p>
          </div>
        <button onClick={handleCheck} className="bg-orange-800 text-white rounded-3xl p-2 my-2 capitalize w-full">confirm order </button>

        </div>
     {value && (  
       <div className="fixed inset-0 bg-black  bg-opacity-50 flex items-center justify-center z-50 ">
        <div className=" bg-white flex flex-col p-6 rounded-3xl w-3/4 max-w-md mx-auto text-center">
...
<path d="M24 3C19.8466 3 15.7865 4.23163 12.333 6.53914C8.8796 8.84665 6.18798 12.1264 4.59854 15.9636C3.0091 19.8009 2.59323 24.0233 3.40352 28.0969C4.21381 32.1705 6.21386 35.9123 9.15077 38.8492C12.0877 41.7861 15.8295 43.7862 19.9031 44.5965C23.9767 45.4068 28.1991 44.9909 32.0364 43.4015C35.8736 41.812 39.1534 39.1204 41.4609 35.667C43.7684 32.2135 45 28.1534 45 24C45 18.4305 42.7875 13.089 38.8493 9.15076C34.911 5.21249 29.5696 3 24 3ZM24 42C20.4399 42 16.9598 40.9443 13.9997 38.9665C11.0397 36.9886 8.73256 34.1774 7.37018 30.8883C6.0078 27.5992 5.65134 23.98 6.34587 20.4884C7.04041 16.9967 8.75474 13.7894 11.2721 11.2721C13.7894 8.75473 16.9967 7.0404 20.4884 6.34587C23.98 5.65133 27.5992 6.00779 30.8883 7.37017C34.1774 8.73255 36.9886 11.0397 38.9665 13.9997C40.9443 16.9598 42 20.4399 42 24C42 28.7739 40.1036 33.3523 36.7279 36.7279C33.3523 40.1036 28.7739 42 24 42Z" fill="#1EA575"/>
        ...
        <h1 className="text-left text-3xl font-bold capitalize ">order confirmed</h1>
        <p className="text-gray-500 capitalize text-left text-sm m-1 ">we hope you enjoy your food</p>
        <div className="bg-orange-50 p-2 rounded-3xl">
    {order.map((item,i)=>{
      return (<div key={i} className="flex my-3 justify-between items-center ">
      <div className="flex gap-2 ">
        <img src={item.image[imageType]} className="w-14 bg-black  rounded-lg" alt="" />
    ...
    <div className="flex justify-between">
          <h3 className="text-gray-800">Order Total</h3>
          <p className="font-bold">${sum}</p>
        </div>
    <button onClick={handleCheck} className="bg-orange-700 text-white rounded-3xl text-center p-2 mt-4 w-full" type="submit">Start New Order</button>
  </div>
        </div>
         ...
         <div className="bg-white flex flex-col items-center shadow-sm  w-full h-36 rounded-sm p-5 text-center ">
